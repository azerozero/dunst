...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use visualops_vision::{capture::capture_window, shapes::detect_shapes};
...
#[cfg(target_os = "macos")]
fn count_kinds(shapes: &[visualops_vision::shapes::Shape]) -> KindCounts {
    let mut counts = KindCounts::default();
    for shape in shapes {
        match shape.kind {
            visualops_vision::shapes::ShapeKind::Rect => counts.rect += 1,
            visualops_vision::shapes::ShapeKind::Bar => counts.bar += 1,
            visualops_vision::shapes::ShapeKind::Circle => counts.circle += 1,
            visualops_vision::shapes::ShapeKind::Line => counts.line += 1,
            visualops_vision::shapes::ShapeKind::Unknown => counts.unknown += 1,
        }
    }
    counts
}
