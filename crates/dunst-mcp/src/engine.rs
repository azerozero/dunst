...
impl Engine {
    ...
    pub fn click_element(&mut self, id: &str, reasoning: Option<&str>) -> dunst_core::Result<AuditEntry> {
        self.act(id, SemanticAction::Click, None, reasoning, None)
    }

    pub fn type_into(&mut self, id: &str, text: &str, reasoning: Option<&str>) -> dunst_core::Result<AuditEntry> {
    ...
    fn click_at_button(
        ...
        let (ox, oy) = dunst_vision::capture::window_bounds(self.target.window_id)
            .map(|(x, y, _, _)| (x, y))
            .unwrap_or((0.0, 0.0));
        let outcome = if dunst_platform::click_web_background(
            self.target.pid,
    ...
    pub fn type_keys(&mut self, text: &str) -> dunst_core::Result<AuditEntry> {
        let outcome = if dunst_platform::type_text_background(
            self.target.pid,
            self.target.window_id,
            text,
        ) {
    ...
    #[cfg(target_os = "macos")]
    pub fn scroll(&mut self, direction: &str, pages: usize) -> dunst_core::Result<AuditEntry> {
        ...
        let mut ok = true;
        for _ in 0..n {
            ok &= dunst_platform::key_web_background(
                self.target.pid,
                self.target.window_id,
                keycode,
                0,
    ...
    /// Non-macOS stub.
    #[cfg(not(target_os = "macos"))]
    pub fn scroll(&mut self, _direction: &str, _pages: usize) -> dunst_core::Result<AuditEntry> {
    ...
    pub fn zoom(&mut self, direction: &str) -> dunst_core::Result<AuditEntry> {
        ...
        let ok = dunst_platform::key_web_background(
            self.target.pid,
            self.target.window_id,
            keycode,
            CMD,
    ...
    fn ocr_screen_fovea(&self, cx: f64, cy: f64) -> Vec<TextHit> {
        ...
        // from the cursor.
        let path = format!("/tmp/dunst_fovea_{}.png", std::process::id());
        ...
        let boxes = dunst_vision::ocr::ocr_image_file(
            &path,
    ...
    pub fn scan_chart(&self, samples: usize) -> dunst_core::Result<ScanResult> {
        ...
        let xs: Vec<f64> = (0..n)
            ...
            .collect();
        let ys = dunst_vision::detect::curve_screen_y(
            &captured.image,
            &captured.geometry,
            &region,
            &xs,
