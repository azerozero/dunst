...
fn cursor_location() -> visualops_core::Result<core_graphics::geometry::CGPoint> {
...
fn usage(message: &str) -> visualops_core::VisualOpsError {
    visualops_core::VisualOpsError::Perception(format!(
        "{message}; usage: cargo run -p visualops-platform --example noforeground_check -- <pid> <window_id>"
    ))
}
