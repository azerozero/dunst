...
use visualops_core::{
    NodeChange, Perceptor, RawAxNode, Role, SceneGraph, Target, WindowRef,
};
use visualops_graph::scene::synth_id;
use visualops_graph::{build_scene_graph, diff};

/// `RawAxNode` builder that can set an `ax_identifier`.
fn raw(role: &str, label: Option<&str>, ax_identifier: Option<&str>, children: Vec<RawAxNode>) -> RawAxNode {
    RawAxNode {
...
fn synth_id_prefers_developer_identifier() {
    ...
    assert_eq!(synth_id(Role::Button, Some("Save"), Some("save-btn"), &[0], &used), "btn_save_btn");

    // A real selector-style identifier is honoured.
    assert_eq!(
        synth_id(Role::MenuItem, Some("Forcer à quitter"), Some("_forceQuitRequested:"), &[7], &used),
...
fn synth_id_ignores_appkit_auto_identifiers() {
    ...
    assert_eq!(synth_id(Role::MenuItem, Some("Copier"), Some("_NS:573"), &[0], &used), "mi_copier");
    assert_eq!(synth_id(Role::Window, Some("Notes – Aucune note"), Some("_NS:6"), &[0], &used), "win_notes_aucune_note");

    // Empty / punctuation-only identifiers are unusable -> fall back to label.
    assert_eq!(synth_id(Role::Button, Some("Save"), Some(""), &[0], &used), "btn_save");
    assert_eq!(synth_id(Role::Button, Some("Save"), Some("###"), &[0], &used), "btn_save");

    // `_NS:` with a non-numeric tail is NOT the auto pattern -> treated as a real
    // identifier (guards the digit-only check).
    assert_eq!(synth_id(Role::Button, Some("Save"), Some("_NS:custom"), &[0], &used), "btn_ns_custom");
    ...
    assert_eq!(id.len(), "menu_".len() + 16, "expected 16-hex path hash: {id}");
}
...
fn identifier_backed_id_is_stable_across_rename() {
    let before = build_scene_graph(
        vec![raw("AXButton", Some("Enregistrer"), Some("save-btn"), vec![])],
        WindowRef::default(),
        1_000,
    );
    let after = build_scene_graph(
        vec![raw("AXButton", Some("Sauvegardé"), Some("save-btn"), vec![])],
        WindowRef::default(),
        2_000,
    );

    // Same id on both sides: derived from the identifier, not the (changed) label.
    assert!(before.get("btn_save_btn").is_some(), "id should derive from identifier");
    assert_eq!(before.roots, after.roots, "root id must be unchanged by the rename");
    ...
    match &d.changes[0] {
        NodeChange::Changed { id, field, before, after } => {
...
fn shared_identifier_collides_with_numeric_suffix() {
    ...
    assert!(graph.get("btn_dup").is_some(), "first sibling takes the base id");
    assert!(graph.get("btn_dup_2").is_some(), "second sibling disambiguates with _2");
    // Both survived into the id-keyed map (no silent drop).
    assert_eq!(
        graph.nodes.values().filter(|n| n.role == Role::Button).count(),
        2
    );
}
...
fn notes_fixture_label_derived_ids_are_unchanged() {
    ...
    for id in [
        ...
        assert!(graph.get(id).is_some(), "label-derived id {id:?} must still be produced");
    }

    // The AppKit-auto ids must NOT have leaked into synthesis.
    for leaked in ["mi_ns_573", "mi_ns_581", "mi_ns_598", "win_ns_6", "outline_ns_50"] {
        assert!(graph.get(leaked).is_none(), "AppKit `_NS:` id {leaked:?} must not be used");
    }
}
...
fn notes_fixture_developer_identifier_ids_are_stable() {
    ...
    for (new_id, old_label_id) in [
        ...
        assert!(graph.get(new_id).is_some(), "identifier-derived id {new_id:?} expected");
        assert!(graph.get(old_label_id).is_none(), "old label id {old_label_id:?} must be gone");
    }
}
